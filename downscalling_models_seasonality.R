...
#Quantiles without the seasonal effect
probs = seq(0, 1, by=0.05) #quantiles to calculate
quant_sim = quantile(sim3, probs=probs)
quant_obs = quantile(obs, probs=probs)
#calculate qantiles

dat_month = merge(quant_month_obs, quant_month_sim, all = TRUE)
dat_season = merge(quant_season_obs, quant_season_sim, all = TRUE)
...
#melt the object so we can easily add the month and seasons 
obs_df <- gts::melt(obs)

#add month and seasons to the object 
# Add month and seasons to the object
obs_df <- obs_df %>%
  mutate(
    ...
    season = factor(
      ...
      levels = c("Summer", "Autumn", "Winter", "Spring") #the 4 factors of seasons (1,2,3,4)
    )
  )

# Indicate the quantiles we wanna compute / num format 
probs <- seq(0, 1, by = 0.05)
quantile_names <- formatC(probs, format = "f", digits = 2)

# Quantiles computation by month 
quant_month_obs <- obs_df %>%
...
# Quantiles computation by month 
quant_month_sim <- sim_df %>%
  group_by(longitude, latitude, month_fact) %>%
  summarise(
    q = list(setNames(quantile(to, probs = probs, na.rm = TRUE), quantile_names)),
    .groups = "drop"
  ) %>%
  tidyr::unnest_longer(q, values_to = "to_quantile", indices_to = "quantile")

#Quantiles computation by season 
quant_season_sim <-sim_df %>%
  group_by(longitude, latitude, season) %>%
  summarise(
    q = list(setNames(quantile(to, probs = probs, na.rm = TRUE), quantile_names)),
    .groups = "drop"
...
library(ggplot2)
ggplot(dat_season, aes(x = season, y = to, group = quantile, color = quantile)) +
  geom_line() + geom_point() +
  theme_minimal()
#visualize the quantiles 

mod_month <- gam(sst_quantile ~ s(as.numeric(month_fact), bs = "cs", k = 12),
                 data = quant_month_obs)
summary(mod_month)
plot(mod_month, shade = TRUE)
...
#Global non linear
mod1.0 = gam(sst ~ s(to, bs="cs", k=40) +ti(lon, k=20) + ti(lat,k=25) +ti(lon,lat,k=15), data=dat)
#tic()
mod1.1 = scam(sst ~ s(to, bs="cs", k=10) +ti(lon, k=20) + ti(lat,k=25) +ti(lon,lat,k=15), data=dat) #scam package 
#toc()
